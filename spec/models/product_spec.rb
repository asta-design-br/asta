...
RSpec.describe Product, type: :model do
  ...
  it 'should have a product description bigger than 50 chars' do
    ...
    expect(@product).to_not be_valid
    expect(@product.errors[:description]).to include('is too short (minimum is 50 characters)')
  end

  it 'should have valid format' do
    @product.product_format = 'Mussum'
    expect(@product).to_not be_valid
    expect(@product.errors[:product_format]).to include('Mussum is not a valid format.')
  end
